#!/usr/bin/env python3
"""Run three web.gobuster trials via dispatch."""
...
# Load the card
card_path = '/home/mark/.securatron/global/tools/web.gobuster.yaml'
...
flags_map = {
    0: '-f --no-error -q -t 10',                          # basic, with slash append
    1: '-f --no-error -q -t 10 -x php,html,txt',          # with file extensions
    2: '-f --no-error -q -t 10 -e',                        # expanded URLs
}

targets = [
    ('scanme.nmap.org', f'Trial 1: basic dir enum (flag=0)'),
    ('scanme.nmap.org', f'Trial 2: with extensions (flag=1)'),
    ('scanme.nmap.org', f'Trial 3: expanded URLs (flag=2)'),
]
...
for i, (target, label) in enumerate(targets):
    ...
    flags = flags_map[i]
    wordlist = card['inputs']['wordlist']['default']
    inputs = {'target': target, 'flags': flags, 'wordlist': wordlist}
    ...
    if artifact_rel:
        ...
        print(f'  Artifact: exists={exists}, size={size} bytes')
        if exists and size > 0:
            content = artifact_path.read_text()[:400]
...
print()
print('=== LEDGER CHECK ===')
ledger_path = Path('/home/mark/.securatron/global/ledger/web.gobuster.trials.jsonl')
